...
const Cart = () => {
	...
	return (
		<div className=' pr-[1.75rem] md:pr-[40px] lg:pr-[3rem] relative 1110:pr-0'>
			<button onClick={handleToggleCart} className={`z-40 relative ${isCartOpened ? 'cart-active' : ''}`}>
			...
			<section ref={cartRef}
				className={`bg-white absolute w-[327px] top-[82px] right-[25px] lg:right-[5px] rounded-[8px] py-[2rem] z-50 px-[1.75rem] lg:px-[2rem]
						${isCartOpened ? '' : 'hidden'}`}>
				{cartItems.length === 0 && (
					<div>
