...
const CartSummary = () => {
  ...
  return (
    <>
      {cartItems.length > 0 && (
        <>
          <section className="px-[1.5rem] mb-[116px] lg:flex-2 lg:w-full lg:max-w-[350px] md:px-0">
            <div className="bg-white px-[1.5rem] md:px-[33px] py-8 rounded-[8px] w-full mt-8 lg:mt-0">
                  ...
                  <li key={item.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="font-bold">
                        ...
                        <p className="text-[.875rem] leading-[25px] opacity-40">{`$ \u00A0${(item.basePrice * item.productQuantity).toLocaleString()}`}</p>
                      </div>
                    </div>
              ...
              <button onClick={handleOpenCheckoutModal}
                className="bg-reddish-orange hover:bg-reddish-hover font-bold text-[.813rem] py-[15px] tracking-[1px] uppercase text-white w-full">
                Continue & Pay
              </button>
            </div>
          </section>
